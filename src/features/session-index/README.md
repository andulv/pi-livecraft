...
Entries additionally summarize what the agent did during the turn: assistant turn count, tool calls (failures noted in parentheses), billed input and output tokens (input totals include cached reads, which are shown as a percentage), and the turn duration. Durations prefer the telemetry measured during the current Pi Livecraft run and otherwise fall back to the span between the user message and the turn's last recorded activity, so reopened sessions still show timings.
