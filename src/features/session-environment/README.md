...
- **Context** — the selected session's model and thinking level, context-window usage with the same pressure thresholds as the composer, and the context files Pi loaded into the system prompt (path and byte size in their own section; each file row expands into its content, sliced from the assembled text via the parts' offsets). The **Assembled prompt** section — a collapsible header like Tools and Skills — expands into the exact text Pi sends, with an owner marker between sections — Pi's default prompt, SDK tools, each extension file, project context files, and unattributed session contributions. Ownership comes from the tool registry (each tool carries its own snippet and guideline texts plus source metadata), and the extension locates each contribution verbatim in the assembled text to publish its char offsets, so the rendered slices concatenate back to the exact prompt. Contributions Pi received without a matching tool are marked as unattributed 'Session' sections.
