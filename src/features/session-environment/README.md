...
- **Skills** — available skills grouped by Pi provenance (scope or source package), with each SKILL.md path, description, and an estimated prompt footprint. The section and source groups expand independently. Livecraft receives only character counts, never skill contents; when the extension has not refreshed its definition data, the widget explicitly requests a refresh instead of showing zero context; token counts use the same four-characters-per-token heuristic as tools;
