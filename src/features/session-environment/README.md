...
- **Skills** — available skills grouped by Pi provenance (scope or source package), with each SKILL.md path, description, and an estimated prompt footprint. The section and source groups expand independently. Livecraft receives only character counts, never skill contents; when Pi has not exposed a loaded definition, the widget marks its size and provenance as pending instead of showing zero context; token counts use the same four-characters-per-token heuristic as tools;
