...
- **Context** — the selected session's model and thinking level, context-window usage with the same pressure thresholds as the composer, the assembled system prompt's size with a component breakdown (custom-prompt flag, guidelines, appended text, tool snippets), and the context files Pi loaded into the system prompt (path and byte size as their own section). Prompt sections are grouped by the owner that contributed them — Pi's default prompt, SDK-registered tools, each extension file, context files, and unattributed session contributions — and each group expands into its sections' text. Ownership comes from the tool registry (each tool carries its own snippet and guideline texts plus source metadata); guideline bullets no active tool claims stay under the unattributed 'Session' owner, and Pi's default prompt shows only its remainder size because it is not reproducible through the public API. The assembled text embeds context-file contents because it is what the next provider request carries.
