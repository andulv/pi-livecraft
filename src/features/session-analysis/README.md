...
The widget is frontend-only. Opening it does not call another model or alter the session.
...
Tool input and output rankings measure serialized input and raw output size, not provider tokens. A tool failure is counted only when Pi reports `isError === true`.

## Ownership and validation

`analyzeSession()` derives the report in linear time from the current session snapshot, Pi session stats, and live telemetry. `SessionAnalysisWidget` owns ranking choices and presentation. `App.tsx` supplies conversation navigation.
