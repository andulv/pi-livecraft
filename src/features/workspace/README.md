...
`useWorkspaceSessions` owns workspace selection and session lists for exactly one selected project. It coordinates refreshes, lazy creation, reopening, renaming and closing, optimistic naming, pending UI requests, and per-project workspace persistence. Selecting a workspace opens the newest listed session. An empty workspace, or the new-session action, selects an in-memory pending session; Pi is created only when its first message is sent. The project repository's main checkout and linked worktrees are the only selectable workspaces in that view.
