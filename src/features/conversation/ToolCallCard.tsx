...
function DiffLineContent({ content, segments }: { content: string; segments?: { text: string; kind: 'added' | 'removed' | 'shared'; highlighted?: boolean }[] }) {
  if (!segments || segments.length === 0) return <pre>{content}</pre>
  return <pre>{segments.map((seg, i) => <span className={seg.highlighted === false ? undefined : `diff-seg-${seg.kind}`} key={i}>{seg.text}</span>)}</pre>
}
