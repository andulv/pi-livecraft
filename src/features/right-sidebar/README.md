...
Panel contracts: [session index](/src/features/session-index/README.md), [quotas](/src/features/quotas/README.md), [session analysis](/src/features/session-analysis/README.md), [session environment](/src/features/session-environment/README.md), and the [Browser system diagnostics](/src/features/browser/README.md). [Terminal](/src/features/terminal/README.md) is an immediate rail action rather than a panel. The session index is the default when no sidebar choice has been saved; an explicit collapsed choice is still respected, and a stored widget that no longer exists (such as the retired `git`) restores the session index. Registry and width behavior are covered by `test/shortcuts.test.ts` and `test/git-sidebar.test.ts`.
