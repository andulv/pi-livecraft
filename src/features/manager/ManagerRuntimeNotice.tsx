...
export function ManagerRuntimeNotice({ activeSession, status, onRestart, onError }: ManagerRuntimeNoticeProps) {
  ...
  return <aside aria-label={label} aria-live="polite" className={`manager-runtime-control${unknown ? ' unknown' : ''}${restarting ? ' restarting' : ''}${hasAction ? ' actionable' : ''}`} tabIndex={hasAction ? undefined : 0}>
    <span aria-hidden="true" className="manager-runtime-control-icon">{unknown ? '⚠' : '↻'}</span>
    <span className="manager-runtime-control-copy">
      <strong>{title}</strong>
      <small>{description}</small>
    </span>
    {hasAction && <button aria-busy={restarting} aria-label="Restart the manager" disabled={disabled} onClick={() => void restart()} type="button">
