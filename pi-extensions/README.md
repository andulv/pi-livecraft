...
- `session-environment.ts` registers the private `/livecraft-environment` command and publishes loaded tools, context files, and the assembled system prompt with its component texts through a versioned status payload.
