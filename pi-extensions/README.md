...
- `shub-agents/` registers one `subagent_<name>` tool per bundled TypeScript agent definition. Each call runs a bounded, isolated, persisted child Pi session; the agent owns its prompt, arguments, tools, model list, and effort levels, while the extension shares process control, budget enforcement, progress, and ownership markers. See [`shub-agents/README.md`](shub-agents/README.md).
