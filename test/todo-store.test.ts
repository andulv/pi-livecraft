...
import {
  ...
  parseTodoStore,
  saveWorkspaceTodos,
} from '../server/features/todos/todo-store.ts'
...
test('persists todo lists independently for each workspace', async () => {
  ...
  try {
    await Promise.all([
      ...
      saveWorkspaceTodos(
        '/workspace/b',
        [{ id: 'other', text: 'Autre tâche', completed: false }],
        path,
      ),
    ])

    assert.deepEqual(await loadWorkspaceTodos('/workspace/a', path), todos)
    assert.deepEqual(await loadWorkspaceTodos('/workspace/b', path), [{
      id: 'other',
      text: 'Autre tâche',
      completed: false,
    }])
...
test('validates todo stores and rejects duplicate or empty items', () => {
  assert.deepEqual(
    parseTodoStore(JSON.stringify({ workspaces: { '/workspace': todos } }))
      .workspaces['/workspace'],
    todos,
  )
  assert.throws(
    () =>
      parseTodoItems([{ id: 'same', text: 'A', completed: false }, {
        id: 'same',
        text: 'B',
        completed: false,
      }]),
  ...
  assert.throws(
    () => parseTodoItems([{ id: 'empty', text: '  ', completed: false }]),
