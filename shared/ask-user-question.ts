...
/** Validates the protocol, bounds, and option uniqueness before exposing it to the interface. */
export function parseAskUserQuestionRequest(value: unknown): AskUserQuestionRequest | null {
  if (
    !isObject(value) || value.protocol !== askUserQuestionProtocol || value
...
/** Verifies that an answer exactly matches the questions and choices allowed by the request. */
export function parseAskUserQuestionResponse(
  value: unknown,
  request: AskUserQuestionRequest,
): AskUserQuestionResponse | null {
  if (!isObject(value) || typeof value.cancelled !== 'boolean' || !Array.isArray(value.answers))
    return null
  ...
  const answers = value.answers.map((answer, index) =>
    parseAnswer(answer, request.questions[index])
  )
...
function parseQuestion(value: unknown): AskUserQuestion | null {
  if (
    !isObject(value) || typeof value.question !== 'string' || typeof value.header !== 'string'
    || typeof value.multiSelect !== 'boolean' || !Array.isArray(value.options)
  ) return null
  if (
    !value.question.trim() || !value.header.trim() || value.options.length < 2
...
function parseOption(value: unknown): AskUserQuestionOption | null {
  if (!isObject(value) || typeof value.label !== 'string' || typeof value.description !== 'string')
    return null
  return value.label.trim() && value.description.trim()
...
function parseAnswer(value: unknown, question: AskUserQuestion): AskUserQuestionAnswer | null {
  if (
    !isObject(value) || typeof value.question !== 'string' || value.question !== question.question
    || !Array.isArray(value.selectedOptions) || !value
      .selectedOptions
      .every((option): option is string => typeof option === 'string')
  ) return null
  ...
  if (
    !value.selectedOptions.every((option) => question.options.some(({ label }) => label === option))
  ) return null
